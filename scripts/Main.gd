...
func _build_ruined_courtyard():
	...
	VisualLibrary.add_part(self, landmark, Vector3(0,3.25,-25), Vector3(0,18,0))
...
func _create_arch(position: Vector3):
	...
	for i in range(7):
		...
		VisualLibrary.add_part(self, block, block_pos, Vector3(90,0,-angle+90))
...
func _create_rubble(position: Vector3):
	for i in range(5):
		...
		VisualLibrary.add_part(self, stone, position + Vector3((i-2)*0.32, size.y*0.5, (i%2)*0.28), Vector3(8*i,17*i,9*i))
